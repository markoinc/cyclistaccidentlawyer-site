    ...
    (set, get) => ({
          ...
          deals: state.deals.map((d) =>
            d.id === id
              ? {
                  ...d,
                  ...updates,
                  kuriosMargin: updates.value
                    ? Math.round(updates.value * 0.3)
                    : d.kuriosMargin,
                }
              : d
          ),
      ...
      addProject: (project) =>
        set((state) => ({
          projects: [
            ...state.projects,
            { ...project, id: `p${Date.now()}`, tasks: [], progress: 0 },
          ],
        })),
      ...
      addToast: (toastOrMessage, type) => {
        // Support both addToast({type, message}) and addToast('msg', 'success')
        const toast =
